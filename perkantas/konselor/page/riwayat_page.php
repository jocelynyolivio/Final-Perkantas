...
<body>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <?php if ($row['catetan_konseling'] == null) : ?>
                                                <div class="input-container">
                                                    <textarea name="catetanKonselor" id="catetan<?= $row['jadwal_konseling_id'] ?>" placeholder="Catatan Konselor" required style="height: 200px; width: 100%; border: 1px solid black;"></textarea>
                                                </div>
                                            <?php else : ?>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <?php if ($row['catetan_konseling'] == null) : ?>
                                                <button type="button" name="simpanCatetan" id="simpanCatetan" class="btn submit2" onclick="simpanCatetan('<?= $row['jadwal_konseling_id'] ?>')">SIMPAN</button>
                                            <?php else : ?>
                                                <button type="button" name="simpanCatetan" id="simpanCatetan" class="btn submit2" disabled>SIMPAN</button>
                                            <?php endif ?>
    ...
    <script>
        function simpanCatetan(id) {
            var idJadwal = id;
            var catetan = $('#catetan'+id).val();
            // alert(catetan)
            if (catetan != null && catetan != "") {
                Swal.fire({
        ...
        function list_load() {
            ...
            xhttp.onload = function() {
                ...
                if (this.responseText == "") {
                    document.getElementById("list-jadwal").innerHTML = '<tr class="odd"><td valign="top" colspan="8" class="dataTables_empty">No data available in table</td></tr>';
                } else {
                    document.getElementById("list-jadwal").innerHTML = this.responseText;
                }
            }
            xhttp.open("GET", "../api/getJadwalInfoPick.php");
...
<script>
    $(document).ready(function() {
        $('.title').html("List Konseling/Psikotest")
        $('.tab-con').show();
