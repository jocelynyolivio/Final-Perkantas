...
$list_file = array($_FILES['filePhoto']);
$list_file_upload = [];
$list_file_path = [];
        ...
        if ($error === 0) {
            if ($img_size > 2000000) {
                ...
                if (in_array($img_ex_lc, $allowed_exs)) {
                    # rename
                    $new_img_name = strtolower($data).'_profil_'.$img_file_name.'.'.$img_ex_lc;

                    # path
                    $path = "../image-data/profil/";
...
if (sizeof($list_file_upload) == 1) {
    ...
    $sql = "UPDATE `konselor_info` SET `konselor_profil`='$list_file_path[0]' WHERE konselor_email = '$check_konselor'";
    mysqli_query($con, $sql);

    $sm = "Foto profil berhasil diunggah!";
